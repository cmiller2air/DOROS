...
class InstanceMask:
    """
    Implements InstanceMask
    """

    def __init__(self, p=0.5) -> None:
        """
        Initializes the InstanceMask with a given probability.

        Args:
            p (float, optional): The probability of applying the InstanceMask augmentation. Must be between 0 and 1.
                                 Default is 0.5.
        """
        self.p = p
    ...
    def __call__(self, labels):
        """
        Applies the InstanceMask augmentation to the given image and instances.
        ...
        if self.p and len(instances.bboxes):
            n = len(instances.bboxes)
            _, w, _ = im.shape  # height, width, channels
            for i in range(n):
                xmin, ymin, xmax, ymax = instances.bboxes[i]
                ...
                if q >=0.5:
                    ...
                    ratio = np.random.uniform(low = 0.05, high = 0.25, size = 1)

                    if p == 0:
                        y_s = ymin + int(box_h*ratio)
                        im[ymin:y_s+1, xmin:xmax+1, :] = 0

                    elif p == 1:
                        y_e = ymax - int(box_h*ratio)
                        im[y_e:ymax+1, xmin:xmax+1, :] = 0

                    elif p == 2:
                        x_s = xmin + int(box_w*ratio)
                        im[ymin:ymax+1, xmin:x_s+1, :] = 0

                    elif p == 3:
                        x_e = xmax - int(box_w*ratio)
                        im[ymin:ymax+1, x_e:xmax+1, :] = 0
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        labels["loc"] = np.concatenate([labels["loc"], labels2["loc"]], 0)
        labels["action"] = np.concatenate([labels["action"], labels2["action"]], 0)
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        if self.direction == "horizontal" and random.random() < self.p:
            ...
            # append for label change because of left light and right light
            for j in range(len(labels["action"])):
                if labels["action"][j][1]==1:
                    key = 1
                elif labels["action"][j][2]==1:
                    key = 2
                else:
                    key = 0
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            for j in random.sample(list(indexes), k=round(self.p * n)):
                ...
                loc = np.concatenate((loc, loc[[j]]), axis=0)
                if action[[j]][0][1] == 1:
                    key = 1
                elif action[[j]][0][2]==1:
                    key = 2
                ...
                if key==1:
                    ...
                    action[[j]][0][1]=1
                    action[[j]][0][2]=0 
                action = np.concatenate((action, action[[j]]), axis=0)
    ...
    def __init__(self, p=1.0):
        ...
        try:
            ...
            T = [
                ...
                # append for weather augmentation
                # A.OneOf([
                ...
                #     A.RandomFog(fog_coef_lower=0.7, fog_coef_upper=0.8, alpha_coef=0.1, p=0.5)
                # ])
            ]
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    if dataset.use_keypoints:
        ...
        if len(flip_idx) == 0 and hyp.fliplr > 0.0:
            hyp.fliplr = 0.0
            LOGGER.warning("WARNING тЪая╕П No 'flip_idx' array defined in data.yaml, setting augmentation 'fliplr=0.0'")
